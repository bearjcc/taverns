import * as React from "react";

function Bread(props) {
  return (
    <svg viewBox="0 0 130.031 65.879" {...props}>
      <defs>
        <linearGradient id="prefix__a">
          <stop offset={0} stopColor="#794000" />
          <stop offset={0.593} stopColor="#94622d" />
          <stop offset={0.731} stopColor="#895014" />
          <stop offset={0.937} stopColor="#341b00" />
          <stop offset={1} stopColor="#412200" />
        </linearGradient>
        <linearGradient
          xlinkHref="#prefix__a"
          id="prefix__b"
          x1={235.664}
          y1={242.366}
          x2={233.519}
          y2={296.383}
          gradientUnits="userSpaceOnUse"
        />
      </defs>
      <g transform="translate(-171.139 -236.837)">
        <path
          d="M228.519 242.725s-6.18-1.524-8.81-1.457c-7.528.194-15.132 1.96-22.224 4.487-7.891 2.811-18.326 12.306-19.652 16.326-1.269 3.738-2.819 10.17-2.066 17.01.347 7.117 7.654 13.235 13.384 14.142 21.285 2.46 42.624 4.482 64.145 5.05 0 0 3.944-1.168 5.808-2.02 5.213-2.383 9.793-5.983 14.9-8.586 4.6-2.344 14.142-6.314 14.142-6.314 4.753-2.122 8.048-5.464 8.586-14.9.192-3.355-3.48-12.943-12.627-16.414-3.835-2.44-11.931-5.985-21.465-6.314 0 0-4.25-.381-7.365-.235-1.156.054-1.385 1.028-2.29.903-2.929-.403-6.727-3.03-8.78-2.94l-13.449.015c-.648-.171-1.353-.001-2.237 1.247z"
          fill="url(#prefix__b)"
          fillRule="evenodd"
          stroke="#380000"
          strokeWidth={1.772}
        />
        <path
          d="M220.09 241.47c-6.045 4.967-6.312 11.447-9.376 17.053 6.846-5.774 10.297-11.197 22.411-16.964h-3.036c-.654 0-1.19.476-1.607 1.428zM245.446 241.737c-12.707 7.997-12.076 14.934-13.928 19.197 6.268-6.04 11.966-12.15 23.75-17.5l-3.75.803c-1.9-.922-4.385-1.577-6.072-2.5z"
          fill="#b6722b"
          fillRule="evenodd"
          stroke="#380000"
          strokeWidth={1.772}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M287.5 251.648c-6.34-1.928-12.743-3.35-19.554-1.518-9.966 2.742-15.72 9.696-21.25 16.875-4.637 6.02-4.179 27.31 5.804 31.16 6.482-1.082 14.044-5.618 21.696-10.446l16.608-7.768c2.74-2.972 6.101-4.557 5.892-14.107.308-1.337-3.226-10.83-9.196-14.196z"
          fill="#e3ac73"
          fillRule="evenodd"
          stroke="#380000"
          strokeWidth={1.772}
        />
        <path
          d="M253.482 270.13c-1.191.239-2.429 2.044-1.875 3.125.49.957 2.366.928 3.214.268.572-.445.63-1.472.358-2.143-.264-.65-1.008-1.388-1.697-1.25zM266.244 262.17c-1.221.19-2.49 1.625-1.922 2.485.503.76 2.425.738 3.295.213.586-.354.645-1.17.366-1.704-.27-.517-1.033-1.103-1.739-.994zM282.162 262.29c-1.59.23-3.241 1.972-2.502 3.015.654.924 3.157.896 4.29.259.762-.43.839-1.42.476-2.068-.353-.628-1.345-1.34-2.264-1.207zM251.6 281.061c-1.192.239-2.43 2.044-1.876 3.125.49.957 2.366.929 3.214.268.572-.444.63-1.471.358-2.143-.264-.65-1.008-1.387-1.697-1.25zM290.194 271.065c-.886.221-1.806 1.898-1.394 2.902.365.889 1.76.862 2.39.249.426-.413.469-1.367.266-1.99-.196-.605-.75-1.29-1.262-1.161zM264.748 272.578c-.888.217-1.812 1.856-1.398 2.837.365.869 1.764.843 2.397.243.426-.403.47-1.336.266-1.945-.196-.59-.751-1.26-1.265-1.135zM277.643 273.762c-.79.175-1.61 1.5-1.243 2.292.325.702 1.569.68 2.131.197.38-.327.417-1.08.237-1.572-.175-.477-.668-1.018-1.125-.917zM273.55 266.29c-.718-.099-1.77.677-1.672 1.397.088.637 1.15.99 1.758.783.41-.139.645-.707.624-1.14-.02-.42-.294-.982-.71-1.04z"
          fill="#ae7338"
          fillRule="evenodd"
          stroke="#562500"
          strokeWidth={1.063}
        />
        <path
          d="M223.929 241.916c-1.667 1.628-3.334 2.526-5 5M249.676 243.562c-2.381 1.092-5.745 2.347-7.411 4.822"
          fill="none"
          stroke="#380000"
          strokeWidth={1.772}
          strokeLinecap="round"
        />
        <path
          d="M175.893 273.255c1.665 2.79 3.473 4.479 5.625 4.286l6.964-.625c3.147-.283 6.116.226 9.286 5.803 3.167 5.575 5.068 3.363 6.696 1.965 4.003-3.44 7.688-4.941 11.518-4.018 4.073.981 4.658 11.115 12.59 2.678 1.598-1.7 3.252-3.412 4.196-1.339 4.044 8.882 6.017 7.658 6.428 5.179.933-5.612 1.69-14.532 4.108-11.518.063 6.623-2.311 14.864 6.696 22.053-20.952-.998-41.548-3.595-62.5-5.535 0 0-7.66-4.081-9.732-8.304-1.537-3.132-3.083-12.649-1.875-10.625z"
          fill="#4e2800"
          fillOpacity={0.107}
          fillRule="evenodd"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={186.057}
          cy={260.592}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={195.97}
          cy={252.826}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={205.187}
          cy={251.943}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={209.48}
          cy={246.26}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={227.916}
          cy={252.069}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={222.36}
          cy={258.761}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={214.784}
          cy={267.726}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={206.702}
          cy={264.443}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={197.232}
          cy={260.15}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={198.116}
          cy={266.337}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={190.287}
          cy={266.969}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={204.43}
          cy={274.166}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={211.501}
          cy={276.565}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={223.117}
          cy={271.009}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={230.315}
          cy={275.555}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={239.785}
          cy={271.388}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={242.184}
          cy={256.109}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={250.013}
          cy={256.109}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={257.084}
          cy={249.165}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={235.366}
          cy={278.585}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={236.502}
          cy={281.995}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={206.324}
          cy={280.479}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={203.925}
          cy={283.51}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={199.758}
          cy={281.237}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={196.096}
          cy={277.575}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={192.055}
          cy={274.797}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={186.878}
          cy={275.555}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={182.585}
          cy={275.429}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={179.302}
          cy={272.524}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={179.428}
          cy={269.115}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={262.766}
          cy={286.667}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={268.195}
          cy={283.005}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={270.721}
          cy={280.101}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={276.656}
          cy={271.388}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={282.464}
          cy={271.388}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={285.242}
          cy={268.358}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={289.535}
          cy={263.938}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={289.535}
          cy={260.024}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={272.489}
          cy={256.615}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={275.898}
          cy={261.16}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={273.878}
          cy={261.034}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={262.513}
          cy={259.392}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={256.958}
          cy={263.559}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={258.852}
          cy={269.241}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={256.2}
          cy={275.681}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={259.357}
          cy={280.101}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={258.22}
          cy={286.035}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={260.241}
          cy={290.202}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={252.917}
          cy={289.697}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={255.316}
          cy={292.854}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={249.76}
          cy={290.707}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={249.381}
          cy={276.691}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={261.503}
          cy={292.98}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={264.786}
          cy={291.212}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={267.817}
          cy={289.066}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={270.216}
          cy={287.929}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={273.751}
          cy={286.414}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={276.15}
          cy={284.646}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={279.055}
          cy={283.636}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={281.58}
          cy={283.005}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={285.116}
          cy={281.111}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={287.893}
          cy={279.469}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={290.166}
          cy={278.459}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={293.576}
          cy={274.04}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={282.464}
          cy={279.722}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={278.171}
          cy={281.237}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={257.715}
          cy={280.101}
          r={0.694}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={221.337}
          cy={282.899}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={220.625}
          cy={279.773}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={222.545}
          cy={281.291}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={224.42}
          cy={283.166}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={225}
          cy={280.8}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={235.536}
          cy={284.371}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={236.205}
          cy={286.559}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={238.214}
          cy={285.487}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={239.196}
          cy={283.389}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={205.938}
          cy={282.318}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={205}
          cy={282.094}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={202.411}
          cy={281.38}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={201.83}
          cy={283.255}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={200.357}
          cy={284.907}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={201.563}
          cy={285.175}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={199.42}
          cy={282.853}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={197.768}
          cy={279.818}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={192.813}
          cy={276.782}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={190.893}
          cy={277.005}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={189.152}
          cy={275.443}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={184.955}
          cy={275.889}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={184.107}
          cy={276.96}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={180.938}
          cy={276.514}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={181.116}
          cy={272.719}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={178.661}
          cy={275.085}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={178.17}
          cy={273.701}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={177.857}
          cy={271.38}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={179.955}
          cy={271.246}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={222.321}
          cy={285.8}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={224.241}
          cy={284.862}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={226.429}
          cy={282.853}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={225.179}
          cy={280.175}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={217.679}
          cy={280.264}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={200.804}
          cy={280.085}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={198.661}
          cy={277.675}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={194.598}
          cy={276.38}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={187.411}
          cy={274.326}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={183.75}
          cy={274.326}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={248.527}
          cy={288.032}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={273.125}
          cy={273.835}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={267.321}
          cy={269.193}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={266.563}
          cy={254.103}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={272.634}
          cy={252.943}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={278.125}
          cy={252.853}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={283.616}
          cy={254.46}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={290.491}
          cy={269.148}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={292.768}
          cy={269.014}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={292.5}
          cy={265.309}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={273.214}
          cy={282.496}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={276.027}
          cy={280.889}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
        <circle
          style={{
            marker: "none",
          }}
          cx={286.25}
          cy={277.809}
          r={0.316}
          color="#000"
          overflow="visible"
          fill="#3c1e00"
        />
      </g>
    </svg>
  );
}

const MemoBread = React.memo(Bread);
export default MemoBread;
