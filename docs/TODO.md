...
- [ ] **Verify `mods/base-game/` loading**: Ensure the mod loading system within the Next.js app correctly loads and utilizes the `mods/base-game/` data via API routes or a designated mod loading mechanism. Remove any redundant static serving of these files from `public/mods/` if not intentional.
...
- [ ] **Clarify `public/data` and `public/mods`**: Determine the necessity of `public/data/` and `public/mods/`. If data is served via API routes, these static copies might be redundant or for fallback, which should be documented.
